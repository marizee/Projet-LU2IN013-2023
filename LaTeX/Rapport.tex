...
\[
n\cdot \sum_{k=1}^{n}k=n \cdot \dfrac{n(n+1)}{2} \Longrightarrow \tilde{O}(n^3)
\]

\subsection{Algorithme d'Horner}

\[
g(a)\text{ mod }f = ((...((((g_{n-1}*a)\text{ mod }f\ +\ g_{n-2})*a)\text{ mod }f\ +\ g_{n-3})...)*a)\text{ mod }f\ +\ g_0
\]
...
\[
n\cdot n \Longrightarrow \tilde{O}(n^2)
\]
